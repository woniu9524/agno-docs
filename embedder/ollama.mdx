...
`OllamaEmbedder` 可用于使用 Ollama 在本地将文本数据嵌入到向量中。

<Note>用于生成嵌入的模型需要本地运行。在此示例中是 `openhermes`，因此您必须[安装 `ollama`](https://ollama.com/download) 并在终端中运行 `ollama pull openhermes`。</Note>
