...
`AzureOpenAIEmbedder` 类用于通过 Azure OpenAI API 将文本数据嵌入到向量中。请在此处获取您的密钥：[https://ai.azure.com/](https://ai.azure.com/)。
...
| `api_version`        | `str`                         | `"2024-02-01"`         | 用于 API 请求的 API 版本。                                           |
...
| `azure_ad_token_provider` | `Any`                         | -                      | 用于获取 Azure AD 令牌的提供程序。                                   |
...
| `openai_client`      | `Optional[AzureOpenAIClient]` | -                      | 用于发出 API 请求的 AzureOpenAIClient 实例。可选。                    |
