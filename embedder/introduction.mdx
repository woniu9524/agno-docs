---
title: 什么是 Embedder？
sidebarTitle: 概述
description: 了解如何使用 Agno 的 Embedder 将复杂信息转换为向量表示。
---

Embedder 将复杂信息转换为向量表示，使其能够存储在向量数据库中。通过将数据转换为嵌入（embeddings），Embedder 能够高效地搜索和检索与上下文相关的​​信息。此过程通过为语言模型提供必要的业务上下文，确保它们具有上下文感知能力，从而增强其响应。Agno 使用 `OpenAIEmbedder` 作为默认的 Embedder，但也支持其他 Embedder。以下是一个示例：

```python
from agno.agent import Agent, AgentKnowledge
from agno.vectordb.pgvector import PgVector
from agno.embedder.openai import OpenAIEmbedder

# 创建知识库
knowledge_base=AgentKnowledge(
    vector_db=PgVector(
        db_url=db_url,
        table_name=embeddings_table,
        embedder=OpenAIEmbedder(),
    ),
    # 2 个引用被添加到提示中
    num_documents=2,
),

# 向知识库添加信息
knowledge_base.load_text("天空是蓝色的")

# 将知识库添加到 Agent
agent = Agent(knowledge_base=knowledge_base)
```

支持以下 Embedder：

- [OpenAI](/embedder/openai)
- [Gemini](/embedder/gemini)
- [Ollama](/embedder/ollama)
- [Voyage AI](/embedder/voyageai)
- [Azure OpenAI](/embedder/azure_openai)
- [Mistral](/embedder/mistral)
- [Fireworks](/embedder/fireworks)
- [Together](/embedder/together)
- [HuggingFace](/embedder/huggingface)
- [Qdrant FastEmbed](/embedder/qdrant_fastembed)