...
Agno 使用 `OpenAIEmbedder` 作为向量数据库的默认嵌入器。`OpenAIEmbedder` 类用于通过 OpenAI API 将文本数据嵌入到向量中。在此处获取您的密钥：[https://platform.openai.com/api-keys](https://platform.openai.com/api-keys)。
...
| `request_params` | `Optional[Dict[str, Any]]` | - | 要包含在 API 请求中的附加参数。 |
| `client_params` | `Optional[Dict[str, Any]]` | - | 用于配置 API 客户端的附加参数。 |
