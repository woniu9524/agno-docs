...
| `max_output_tokens`           | `Optional[int]`                   | `None`               | 在响应输出中生成的最大 token 数。                      |
| `metadata`                    | `Optional[Dict[str, Any]]`        | `None`               | 要随请求包含的其他元数据。                                      |
| `parallel_tool_calls`         | `Optional[bool]`                  | `None`               | 是否允许并行工具调用。                                                 |
| `reasoning`                   | `Optional[Dict[str, Any]]`        | `None`               | 用于启用和控制响应中推理/思考的参数。                                       |
| `store`                       | `Optional[bool]`                  | `None`               | 是否存储此响应请求的输出以进行模型蒸馏或评估。                                     |
...
| `default_headers`             | `Optional[Dict[str, str]]`        | `None`               | 要包含在所有请求中的默认标头。                                           |
| `default_query`               | `Optional[Dict[str, str]]`        | `None`               | 要包含在所有请求中的默认查询参数。                                  |
| `http_client`                 | `Optional[httpx.Client]`          | `None`               | 可选的预配置 HTTP 客户端。                                         |
| `client_params`               | `Optional[Dict[str, Any]]`        | `None`               | 客户端配置的其他参数。                                       |
