...
| `dimension`      | `int`                                  | -          | 嵌入的维度                                                        |
| `spec`           | `Union[Dict, ServerlessSpec, PodSpec]` | -          | 索引规范                                                                         |
| `embedder`       | `Optional[Embedder]`                   | `None`     | 用于创建嵌入的 Embedder 实例（如果未提供，则默认为 OpenAIEmbedder） |
| `metric`         | `Optional[str]`                        | `"cosine"` | 用于相似性搜索的度量                                                  |
| `additional_headers` | `Optional[Dict[str, str]]`             | `None`     | 传递给 Pinecone 客户端的附加标头                                      |
| `pool_threads`   | `Optional[int]`                        | `1`        | 用于 Pinecone 客户端的线程数                                   |
