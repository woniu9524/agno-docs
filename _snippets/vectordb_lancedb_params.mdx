| 参数     | 类型           | 默认值 | 描述                                                                                                                               |
| -------- | -------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `uri`    | `str`          | -       | 要连接的 URI。                                                                                                                     |
| `table`  | `LanceTable`   | -       | 要使用的 Lance 表。                                                                                                                |
| `table_name` | `str`          | -       | 要使用的表的名称。                                                                                                                 |
| `connection` | `DBConnection` | -       | 要使用的数据库连接。                                                                                                               |
| `api_key` | `str`          | -       | 要使用的 API 密钥。                                                                                                                |
| `embedder` | `Embedder`     | -       | 要使用的嵌入器。                                                                                                                   |
| `search_type` | `SearchType`   | vector  | 要使用的搜索类型。                                                                                                                 |
| `distance` | `Distance`     | cosine  | 要使用的距离。                                                                                                                     |
| `nprobes` | `int`          | -       | 要使用的探针数。[更多信息](https://lancedb.github.io/lancedb/ann_indexes/#use-gpu-to-build-vector-index)                               |
| `reranker` | `Reranker`     | -       | 要使用的重排器。[更多信息](https://lancedb.github.io/lancedb/hybrid_search/eval/)                                                  |
| `use_tantivy` | `bool`         | -       | 是否使用 tantivy。                                                                                                                 |