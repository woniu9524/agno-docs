...
| `function_declarations` | `Optional[List[FunctionDeclaration]]` | `None`                   | 函数声明列表，用于模型。                                 |
| `generation_config`  | `Optional[Any]`                       | `None`                   | 文本生成的配置。                                         |
| `safety_settings`    | `Optional[Any]`                       | `None`                   | 模型的安全设置。                                         |
| `generative_model_kwargs` | `Optional[Dict[str, Any]]`           | `None`                   | 生成模型的可选关键字参数。                               |
...
| `temperature`        | `Optional[float]`                     | `None`                   | 控制输出的随机性。较高的值（例如 0.8）会使输出更随机，而较低的值（例如 0.2）会使输出更聚焦和确定性。 |
| `top_p`              | `Optional[float]`                     | `None`                   | nucleus sampling 参数。仅考虑累积概率超过此值的 token。 |
| `top_k`              | `Optional[int]`                       | `None`                   | 仅为文本生成考虑 top k 个 token。                        |
| `max_output_tokens`  | `Optional[int]`                       | `None`                   | 在响应中生成的最大 token 数。                            |
| `stop_sequences`     | `Optional[list[str]]`                | `None`                   | 模型应停止生成更多 token 的序列列表。                    |
| `logprobs`           | `Optional[bool]`                      | `None`                   | 是否返回输出 token 的 log 概率。                         |
| `presence_penalty`   | `Optional[float]`                     | `None`                   | 根据 token 在文本中是否出现来惩罚新 token。              |
| `frequency_penalty`  | `Optional[float]`                     | `None`                   | 根据 token 在文本中出现的频率来惩罚新 token。            |
