...
| `name` | `str` | - | 此 LLM 的名称。注意：这不会发送到 LLM API。 |
| `metrics` | `Dict[str, Any]` | - | 为此 LLM 收集的指标。注意：这不会发送到 LLM API。 |
...
| `tool_choice` | `Union[str, Dict[str, Any]]` | - | 控制模型调用哪个（如果有）函数。“none”表示模型不会调用函数，而是生成消息。“auto”表示模型可以在生成消息或调用函数之间进行选择。通过 `{"type": "function", "function": {"name": "my_function"}}` 指定特定函数会强制模型调用该函数。“none”是默认值，当没有函数存在时。“auto”是默认值，如果存在函数。 |
| `run_tools` | `bool` | `True` | 如果为 True，则运行工具。 |
| `show_tool_calls` | `bool` | - | 如果为 True，则在响应中显示工具调用。 |
| `functions` | `Dict[str, Function]` | - | 从工具中提取的函数。注意：这些不会发送到 LLM API，仅用于执行。 |
