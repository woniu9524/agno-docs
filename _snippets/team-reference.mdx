...
| `mode` | `Literal["route", "coordinate", "collaborate"]` | `"coordinate"` | 团队操作模式 |
...
| `team_session_id` | `Optional[str]` | `None` | 在团队本身是另一个团队的成员的情况下 |
| `session_name` | `Optional[str]` | `None` | 会话名称 |
| `session_state` | `Optional[Dict[str, Any]]` | `None` | 会话状态（存储在数据库中以跨运行持久化） |
...
| `add_datetime_to_instructions` | `bool` | `False` | 如果为 True，则将当前日期时间添加到指令中，以让团队了解时间感 |
| `add_location_to_instructions` | `bool` | `False` | 如果为 True，则将当前位置添加到指令中，以让团队了解位置感 |
...
| `enable_agentic_context` | `bool` | `False` | 如果为 True，则启用团队代理来更新团队上下文并自动将团队上下文发送给成员 |
| `share_member_interactions` | `bool` | `False` | 如果为 True，则将所有先前的成员交互发送给成员 |
| `get_member_information_tool` | `bool` | `False` | 如果为 True，则添加一个获取团队成员信息的工具 |
| `search_knowledge` | `bool` | `True` | 添加一个搜索知识库的工具（也称为 Agentic RAG） |
...
| `tool_choice` | `Optional[Union[str, Dict[str, Any]]]` | `None` | 控制由团队模型调用哪个（如果有）工具 |
...
| `memory` | `Optional[Union[TeamMemory, Memory]]` | `None` | 团队的内存 |
| `enable_agentic_memory` | `bool` | `False` | 启用代理内存管理 |
| `enable_user_memories` | `bool` | `False` | 如果为 True，则代理在运行结束时创建/更新用户记忆 |
| `add_memory_references` | `Optional[bool]` | `None` | 如果为 True，则代理在响应中添加对用户记忆的引用 |
| `enable_session_summaries` | `bool` | `False` | 如果为 True，则代理在运行结束时创建/更新会话摘要 |
| `add_session_summary_references` | `Optional[bool]` | `None` | 如果为 True，则代理在响应中添加对会话摘要的引用 |
| `add_history_to_messages` | `bool` | `False` | 如果为 True，则将聊天历史记录中的消息添加到发送给模型的消息列表中。 |
| `num_history_runs` | `int` | `3` | 要包含在消息中的历史运行次数 |
| `storage` | `Optional[Storage]` | `None` | 团队的存储 |
| `extra_data` | `Optional[Dict[str, Any]]` | `None` | 与此团队一起存储的额外数据 |
| `reasoning` | `bool` | `False` | 启用团队的推理功能 |
...
| `stream_member_events` | `bool` | `True` | 流式传输团队成员的成员事件 |
| `store_events` | `bool` | `False` | 将流式传输的事件存储在 TeamRunResponse 上 |
| `events_to_skip` | `Optional[List[Union[RunEvent, TeamRunEvent]]]` | `None` | 指定存储在 TeamRunResponse 上的事件时要跳过的事件类型 |
| `app_id` | `Optional[str]` | `None` | 可选的应用 ID。表示此团队是应用程序的一部分 |
| `debug_mode` | `bool` | `False` | 启用调试日志 |
| `show_members_responses` | `bool` | `False` | 启用成员日志 - 为团队和成员设置 debug_mode |
| `monitoring` | `bool` | `False` | 将团队信息记录到 agno.com 进行监控 |
| `telemetry` | `bool` | `True` | 记录最少的遥测数据以进行分析 |
...
| `get_member_information` | 获取团队成员的信息 |
| `get_team_history` | 获取团队聊天历史记录 |
| `search_knowledge_base` | 在知识库中搜索信息 |
| `load_session` | 从数据库加载现有会话或创建新会话 |
| `rename_session` | 重命名当前会话 |
| `delete_session` | 删除会话 |
| `add_image` | 将图像添加到团队会话 |
| `add_video` | 将视频添加到团队会话 |
| `add_audio` | 将音频添加到团队会话 |
| `get_images` | 获取团队会话中的所有图像 |
...
- `memory`: 团队内存存储
- `enable_agentic_memory`: 启用代理内存管理
...
- `num_history_runs`: 要包含在消息中的历史运行次数
