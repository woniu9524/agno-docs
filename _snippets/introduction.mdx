---
title: 什么是 Agent？
description: Agent 是智能程序，可以通过采取行动来完成复杂任务。使用它们可以自动化可重复的工作流程并构建新的用户体验。
sidebarTitle: 简介
---
...
- 将知识存储在向量数据库中，以领域专业知识补充其训练数据。

![Agent Architecture](/images/agent.png)

## 示例：研究 Agent

让我们创建一个 Agent，它可以搜索网络并阅读热门链接来撰写研究报告。我们使用 `description` 和 `instructions` 来**“提示”** Agent。
...
<Note>
在底层，description 和 instructions 被转换为系统提示，并将输入作为用户提示传递。

使用 `debug_mode=True` 查看 Agent 使用的原始消息。
...
`Agent.run()` 函数返回一个 `RunResponse` 对象，或者在 `stream=True` 时返回一个 `RunResponse` 对象流。
