...
| `local` | `bool` | `False` | 是否使用本地 Weaviate 实例而不是云实例。 |
| `collection` | `str` | `"default"` | 在 Weaviate 中使用的集合名称。 |
...
| `embedder` | `Optional[Embedder]` | `None` | 用于生成向量嵌入的嵌入器。如果未提供，则默认为 OpenAIEmbedder。 |
| `search_type` | `SearchType` | `SearchType.vector` | 要执行的搜索类型（vector、keyword 或 hybrid）。 |
| `reranker` | `Optional[Reranker]` | `None` | 用于改进搜索结果的可选重排器。 |
| `hybrid_search_alpha` | `float` | `0.5` | 混合搜索中向量搜索和关键字搜索之间的权重（0.0 = 仅关键字，1.0 = 仅向量）。 |