...
| `memory_manager` | `Optional[MemoryManager]` | 记忆操作的管理器 | `None` |
...
| `create_session_summary` | 从存储的会话运行创建会话摘要 | `session_id: str, user_id: Optional[str] = None` |
| `acreate_session_summary` | 从存储的会话运行创建会话摘要 (异步) | `session_id: str, user_id: Optional[str] = None` |
...
| `last_updated` | `Optional[datetime]` | 记忆最后更新时间 | `None` |
...
| `last_updated` | `Optional[datetime]` | 摘要最后更新时间 | `None` |
...
| 参数 | 类型 | 描述 | 默认值 |
|-----------|------|-------------|---------|
| `model` | `Optional[Model]` | 用于总结会话的模型 | `None` |
