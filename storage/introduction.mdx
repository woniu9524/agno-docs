...
Use **Storage** to save Agent sessions and state to a database or file.

<Tip>
**Why we need Agent Storage?**

Agents are ephemeral and the built-in memory only lasts for the current execution cycle.

In production environments, you'll serve your Agents via an API and need to continue sessions in subsequent requests. Storage persists the session history and state in a database and enables us to pick up where we left off.
</Tip>
