---
title: Mongo Agent Storage
sidebarTitle: MongoDB
---

Agno supports using MongoDB as a storage backend for Agents using the `MongoAgentStorage` class.

## Usage

You need to provide either `db_url` or `client`. The following example uses `db_url`.

```python storage.py
from agno.storage.agent.mongodb import MongoAgentStorage

db_url = "mongodb://ai:ai@localhost:27017/agno"

# Create a storage backend using the Mongo database
storage = MongoAgentStorage(
    # store sessions in the agent_sessions collection
    collection_name="agent_sessions",
    db_url=db_url,
)

# Add storage to the Agent
agent = Agent(storage=storage)
```

## Params

<Snippet file="storage-mongodb-params.mdx" />

## Developer Resources

- View [Cookbook](https://github.com/phidatahq/agno/blob/main/cookbook/agent_concepts/storage/mongodb_storage.py)
