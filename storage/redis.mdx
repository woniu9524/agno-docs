...
Agno 支持使用 Redis 作为 Agent 的存储后端，通过 `RedisStorage` 类实现。

## 用法

### 运行 Redis

安装 [Docker Desktop](https://docs.docker.com/desktop/install/mac-install/)，然后使用以下命令在端口 **6379** 上运行 **Redis**：
...
storage = RedisStorage(
    prefix="agno_test",    # 用于 Redis 键的前缀，为会话提供命名空间
