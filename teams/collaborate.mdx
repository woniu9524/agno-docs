...
在**协作模式**下，所有团队成员会同时响应用户查询。这使得团队协调员能够审查团队是否就某个特定主题达成共识，然后将所有团队成员的响应综合成一个单一的响应。

当与 `async await` 一起使用时，这种模式尤其有用，因为它允许各个成员并发响应，协调员可以异步地综合这些响应。
...
1. 团队收到用户查询
2. 所有团队成员都会收到查询。同步运行时，这会逐个发生。异步运行时，这会并发发生。
3. 每个团队成员都会产生一个输出
4. 协调员审查输出并将其综合成一个单一的响应
