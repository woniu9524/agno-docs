...
- **Memory Handling for Google Provider**: Fixed an issue in `monitoring_data` where memory was removed for all providers, causing blank titles on Agno.app; now only modifies memory for Google provider.
