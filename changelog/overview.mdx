...
- **Memory Handling for Google Provider**: Fixed an issue in `monitoring_data` where memory was removed for all providers, causing blank titles on app.agno.com; now only modifies memory for Google provider.
...
- **Baidu Search Tool**: Introducing Baidu Search integration for enhanced search capabilities in Agno workflows.
...
- **Agno Tool Templates**: Integrated new templates and added a comprehensive cookbook.
