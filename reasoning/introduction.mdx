...
Reasoning Tools is a simple, yet effective approach to add reasoning to non-reasoning models. By giving any model a "think" tool, we provide a dedicated space for structured thinking, improving the model's reasoning capabilities.
...
Reasoning Agents are a new type of multi-agent system developed by Agno that combines chain of thought reasoning with tool use. You can enable reasoning on any Agent by setting `reasoning=True`.
