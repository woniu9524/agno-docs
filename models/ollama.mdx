...
使用 Ollama 在本地运行大型语言模型

[Ollama](https://ollama.com) 是一个在本地运行模型的绝佳工具。

Ollama 支持多种开源模型。在此处查看模型库：[Ollama 库](https://ollama.com/library)。
...
安装 [ollama](https://ollama.com) 并使用以下命令运行模型：
...
<Note> 在此处查看更多示例：[../examples/models/ollama](../examples/models/ollama)。 </Note>
