...
sidebarTitle: 入门指南
description: 一个精美、开源的界面，用于与 AI Agent 交互
---
...
Agno 提供了一个精美的 UI，用于与您的 Agent 交互，完全开源，可免费使用和构建。它是一个简单的界面，允许您与 Agent 聊天，查看它们的记忆、知识等。

<Note>
不会将任何数据发送到 [agno.com](https://app.agno.com)，所有 Agent 数据都存储在本地的 sqlite 数据库中。
</Note>

开源 Agent UI 是使用 Next.js 和 TypeScript 构建的。在 [Agent Playground](/introduction/playground) 成功之后，社区要求提供一个自托管的替代方案，我们做到了！
...
输入 `y` 创建一个新项目，安装依赖项，然后使用以下命令运行 agent-ui：

```bash
cd agent-ui && npm run dev
```

打开 [http://localhost:3000](http://localhost:3000) 查看 Agent UI，但请记住连接到您的本地 Agent。
...
让我们从一个本地 playground 服务器开始。创建一个文件 `playground.py`
