---
title: 监控与调试
description: 实时监控您的 Agent、Team 和 Workflows。
---

# 监控

您可以在 [app.agno.com](https://app.agno.com) 上实时跟踪您的 Agent。

## 认证

通过 [agno.com](https://app.agno.com) 进行认证，开始监控您的会话。
请查看 [认证指南](how-to/authentication) 以了解如何使用 Agno 进行认证。

## 启用监控

通过设置 `AGNO_MONITOR=true` 为单个 Agent 或所有 Agent 全局启用监控。

### 为特定 Agent

```python
agent = Agent(markdown=True, monitoring=True)
```

### 为所有 Agent 全局启用

```bash
export AGNO_MONITOR=true
```

## 监控您的 Agent

运行您的 Agent，并在 [会话页面](https://app.agno.com/sessions) 上查看会话。

<Steps>

<Step title="创建一个包含示例代码的文件">

```python monitoring.py
from agno.agent import Agent

agent = Agent(markdown=True, monitoring=True)
agent.print_response("Share a 2 sentence horror story")
```

</Step>

<Step title="运行您的 Agent">

```shell
python monitoring.py
```

</Step>

<Step title="查看您的会话">

在 [app.agno.com/sessions](https://app.agno.com/sessions) 查看您的会话

<img height="200" src="/images/monitoring.png" style={{ borderRadius: "8px" }} />

</Step>

</Steps>

<Info>遇到问题？请查看我们的 [故障排除指南](/faq/cli-auth)</Info>

## 调试日志

想查看系统提示、用户消息和工具调用吗？

Agno 内置了一个调试器，可以在终端中打印调试日志。为任何 Agent 设置 `debug_mode=True` 或在您的环境中设置 `AGNO_DEBUG=true`。

```python debug_logs.py
from agno.agent import Agent
from agno.models.anthropic import Claude
from agno.tools.yfinance import YFinanceTools

agent = Agent(
    model=Claude(id="claude-sonnet-4-20250514"),
    tools=[YFinanceTools(stock_price=True)],
    instructions="Use tables to display data. Don't include any other text.",
    markdown=True,
    debug_mode=True,
)
agent.print_response("What is the stock price of Apple?", stream=True)
```

运行 Agent 以在终端中查看调试日志：

```shell
python debug_logs.py
```

<video
   autoPlay
   muted
   controls
   className="w-full aspect-video"
   style={{ borderRadius: '8px' }}
   src="/videos/debug_logs.mp4"
></video>