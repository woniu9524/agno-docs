...
Using an Agent to dynamically search for information at runtime is called **Agentic Search** and the process of **searching, reasoning and responding** is known as Agentic RAG.

The Model triggers the `search` query but it is the quality of the search results that determine the quality of the Agent's response. You may use semantic search, full text search or hybrid search. Combine Hybrid Search with Reranking and you get best in class knowledge retrieval.
...
Here's a working example that uses Hybrid Search + Reranking - which is state of the art for Agentic Search.
