...
description: 了解如何将 LlamaIndex 检索器或向量存储用作知识库。
...
| 参数       | 类型             | 默认值 | 描述                                                 |
| ------------- | ---------------- | ------ | ---------------------------------------------------- |
| `retriever`   | `BaseRetriever`  | `None` | LlamaIndex 检索器，用于查询知识库。                  |
| `loader`      | `Optional[Callable]` | `None` | 可选的调用函数，用于将文档加载到知识库中。 |

`LlamaIndexKnowledgeBase` 是 [AgentKnowledge](/reference/knowledge/base) 类的子类，并拥有与其相同的参数。
