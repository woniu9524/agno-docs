...
| `reader` | `Union[PDFReader, PDFImageReader]` | `PDFReader()` | 一个 `PDFReader` 或 `PDFImageReader`，用于将 `PDF` 转换为向量数据库的 `Documents`。 |

`PDFKnowledgeBase` 是 [AgentKnowledge](/reference/knowledge/base) 类的子类，可以访问相同的参数。

## 开发者资源

- 查看 [同步加载食谱](https://github.com/agno-agi/agno/blob/main/cookbook/agent_concepts/knowledge/pdf_kb.py)
- 查看 [异步加载食谱](https://github.com/agno-agi/agno/blob/main/cookbook/agent_concepts/knowledge/pdf_kb_async.py)