...
下面是使用 `agno` 库实现自定义检索器函数的详细示例。此示例演示了如何设置包含 PDF 文档的知识库、定义自定义检索器，以及如何将其与代理一起使用。
...
def retriever(
    ...
    try:
        ...
        print(f"向量数据库搜索时出错: {str(e)}")
        return None

def main():
    """演示代理用法的主函数。"""
    # 使用自定义检索器初始化代理
    # 请记住设置 search_knowledge=True 以使用 agentic_rag 或添加 add_reference=True 以使用传统 RAG
    # 当您添加知识库时，search_knowledge=True 是默认值，但在此处是必需的
    agent = Agent(
        retriever=retriever,
...
2.  **加载知识库**：知识库使用 PDF 文档加载。此步骤涉及将文档转换为嵌入并将其存储在向量数据库中。

3.  **自定义检索器函数**：定义 `retriever` 函数来处理文档检索。它接收一个查询，将其转换为嵌入，并在向量数据库中搜索相关文档。
