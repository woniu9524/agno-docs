---
title: Youtube知识库
description: 了解如何在知识库中使用YouTube视频字幕。
sidebarTitle: Youtube
---

**YouTubeKnowledgeBase** 会迭代一个YouTube URL列表，提取视频字幕，将它们转换为向量嵌入，并将它们加载到向量数据库中。
...
| 参数    | 类型                      | 默认值 | 描述                                                                                             |
| ------- | ------------------------- | ------ | ------------------------------------------------------------------------------------------------ |
| `urls`  | `List[str]`               | `[]`   | 要读取的视频的URL                                                                                |
| `reader`| `Optional[YouTubeReader]` | `None` | 一个`YouTubeReader`，它读取URL中的视频字幕，并将它们转换为`Documents`以供向量数据库使用。 |

`YouTubeKnowledgeBase` 是 [AgentKnowledge](/reference/knowledge/base) 类的一个子类，并且可以访问相同的参数。
