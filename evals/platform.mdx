---
title: 在 Agno 平台上的评估
description: 您可以在 Agno 平台上跟踪您的评估运行
sidebarTitle: 平台
---
...
除了在 CLI 上运行评估外，您还可以在 Agno 平台上跟踪它们。这有助于跟踪结果并与您的团队共享。
请按照以下步骤操作：

<Steps>

<Step title="认证">
  您可以使用您的 CLI 或 API 密钥进行认证。
...
<Step title="跟踪您的评估">
  在运行评估时，将 `monitoring` 设置为 `True` 以在 Agno 平台上跟踪其所有运行：
  ```python
...
<Step title="查看您的评估">
    您现在可以在 Agno 平台上的 [app.agno.com/evaluations](https://app.agno.com/evaluations) 查看您的评估。
</Step>
</Steps>
