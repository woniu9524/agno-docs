...
Agno 使用 [docker-py](https://github.com/docker/docker-py) 来运行容器，如果 `/var/run/docker.sock` 缺失或权限不正确，它将无法连接到 Docker。
...
**如果以上方法均无效且 `/var/run/docker.sock` 存在：**

- 为您的用户授予对 `/var/run/docker.sock` 文件的权限：

```shell
sudo chown $USER /var/run/docker.sock
```

- 为您的用户授予对 docker 组的权限：
...
- [Stackoverflow 回答](https://stackoverflow.com/questions/48568172/docker-sock-permission-denied/56592277#56592277)