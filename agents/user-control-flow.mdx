...
Agno 中的用户控制流使您能够实现“人工干预”模式，在代理执行期间需要人工监督和输入。这对于以下方面至关重要：
...
1. **用户确认**：在执行工具调用前需要用户明确批准
2. **用户输入**：在执行期间从用户那里收集特定信息
3. **动态用户输入**：让代理在需要时收集用户输入
4. **外部工具执行**：执行代理控制之外的工具

## 暂停代理执行

用户控制流会中断代理的执行并需要人工监督。然后可以通过调用 `continue_run` 方法来继续运行。
...
`continue_run` 方法会继续代理暂停时的状态。您也可以将特定运行的 `run_response` 或 `run_id` 传递给 `continue_run` 方法。

## 用户确认

用户确认允许您在继续执行工具调用前暂停执行并要求用户明确批准。这对于以下情况很有用：
...
用户输入流允许您在执行期间从用户那里收集特定信息。这对于以下情况很有用：
...
`RunResponse` 对象有一个工具列表，在 `requires_user_input` 的情况下，需要输入的工具将填充 `user_input_schema`。
...
您还可以指定哪些字段应由用户填写，而代理将提供其余字段。

```python

# 您可以指定 `user_input_fields`，将所有字段留空以供用户提供
...
此模式为代理提供了指示何时需要用户输入的工具。它非常适合：

- 用户与代理的交互方式未知的情况
- 当您希望与用户进行类似表单的交互时

在下面的示例中，我们使用了一个专用工具来允许代理在需要时收集用户反馈。
...
# 我们使用 while 循环来继续运行，直到代理对用户输入感到满意为止
...
外部工具执行允许您在代理控制之外执行工具。这对于以下情况很有用：
...
# 我们必须创建一个具有正确名称、参数和文档字符串的工具，以便代理知道要调用哪个。
...
if run_response.is_paused:
    for tool in run_response.tools_awaiting_external_execution:
        if tool.tool_name == execute_shell_command.name:
            ...
            # 我们自己执行工具。您可以执行任何函数或进程，并使用 tool_args 作为输入。
            result = execute_shell_command.entrypoint(**tool.tool_args)
            # 我们必须将结果设置在工具执行对象上，以便代理可以继续
            tool.result = result
...
1. **清理用户输入**：始终验证和清理用户输入，以防止安全漏洞。
2. **错误处理**：始终为用户输入和外部调用实现适当的错误处理。
3. **输入验证**：在处理用户输入之前进行验证。
