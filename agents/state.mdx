...
You can pass state across multiple runs using `session_state` dictionary. The dictionary is passed to the Agent as a parameter, and is available to all tools. `session_state` is also stored with Agent sessions, so is persisted in a database. Meaning, it is available across multiple runs.
...
<Note>
The keys of the `session_state` dictionary are available in the `description` and `instructions` as variables. Use it to add the shopping_list to the instructions directly.
</Note>
