...
You can pass a state dictionary across multiple runs using `session_state`. The dictionary is passed to the Agent as a parameter, and is available to all tools. `session_state` is also stored with Agent sessions, so is persisted in a database. Meaning, it is available across multiple runs.
