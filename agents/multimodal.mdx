...
Agno 代理支持文本、图像、音频和视频输入，并能生成文本、图像、音频和视频输出。如需完整概览，请查看[兼容性矩阵](/models/compatibility)。

## 代理的多模态输入

让我们创建一个能够理解图像并在需要时进行工具调用的代理。
...
<Note>目前 Agno 仅支持将视频作为 Gemini 模型的输入。</Note>
...
agent.print_response("告诉我关于这段视频的内容", videos=[Video(filepath=video_path)])
```
...
image_agent = Agent(
    ...
    instructions="当用户要求你创建图像时，请使用 `create_image` 工具来创建图像。",
    markdown=True,
    show_tool_calls=True,
)

image_agent.print_response("生成一只白色暹罗猫的图像")
...
以下示例演示了如何同时获取文本和音频响应。代理将以文本和可以保存到文件的音频字节进行响应。
