...
## 概述
当您在 Agno 中运行代理时，您收到的响应（**RunResponse**）包含有关运行的详细指标。这些指标可帮助您了解资源使用情况（如**token 使用量**和**时间**）、性能以及模型和工具调用的其他方面。

指标可在多个级别上获得：
- **每个消息**：每条消息（助手、工具等）都有自己的指标。
- **每个工具调用**：每次工具执行都有自己的指标。
...
- `Message.metrics`：每条消息（助手、工具等）的指标。
</Note>

## 示例用法
假设您有一个执行某些任务的代理，并且您想在运行后分析指标。以下是如何访问和打印指标的方法：

您运行以下代码来创建代理并使用以下配置运行它：
```python
...
本节提供每次工具执行的指标。它包括有关单个工具调用资源使用情况和性能的详细信息。

![Tool Run Message Metrics](../images/tools-run-message-metrics.png)

### 消息指标

在这里，您可以看到代理每次消息响应的指标。所有“助手”响应都将具有此类指标，帮助您了解消息级别的性能和资源使用情况。

![Agent Run Message Metrics](../images/agent-run-message-metrics.png)

### 聚合运行指标

聚合指标提供了整个运行的全面视图。这包括所有消息和工具调用的摘要，让您对代理的性能和资源使用情况有一个总体了解。

![Aggregated Run Metrics](../images/aggregated-run-metrics.png)
