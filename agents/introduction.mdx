...
Let's create a research agent that can search the web using DuckDuckGo, scrape the top links using Newspaper4k and write a research report for us. Ideally we'll use specialized tools (like Exa) for this but let's start with the free tools first.
...
<Note>
The description and instructions are converted to the system prompt and the input (e.g. `"The current state of artificial intelligence regulation"`) is passed as the user prompt.

Set `debug_mode=True` to view logs behind the scenes.
</Note>
