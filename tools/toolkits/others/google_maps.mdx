---
title: "Google Maps"
description: "用于与 Google Maps 服务进行交互的工具，包括地点搜索、路线、地理编码等"
---

**GoogleMapTools** 使 Agent 能够与各种 Google Maps 服务进行交互，以进行基于位置的操作，包括地点搜索、路线、地理编码等。

## 先决条件

以下示例需要 `googlemaps` 库和一个 API 密钥，API 密钥可以从 [Google Cloud Console](https://console.cloud.google.com/projectselector2/google/maps-apis/credentials) 获取。
...
更多示例，请参阅 [Google Maps Tools Examples](/examples/concepts/tools/others/google_maps)。
...
| `get_directions` | 获取地点之间的路线。参数：`origin` (str)、`destination` (str)，可选 `mode` (str) 用于出行模式，可选 `avoid` (List[str]) 用于要避免的特征。返回路线信息。 |
...
| `get_elevation` | 获取某个地点的高度。参数：`lat` (float)、`lng` (float)。返回海拔数据。 |
| `get_timezone` | 获取某个地点的时间区。参数：`lat` (float)、`lng` (float)，可选 `timestamp` (datetime)。返回时区信息。 |

## Rate Limits

Google Maps API 有使用限制和配额，具体取决于服务和结算计划。请参阅 [Google Maps Platform pricing](https://cloud.google.com/maps-platform/pricing) 获取详细信息。
