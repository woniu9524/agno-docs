...
**MLX Transcribe** 是一个使用 MLX Whisper 转录音频文件的工具。
...
4. **下载示例音频** (可选)
   - 访问 [audio-samples](https://audio-samples.github.io/) (作为示例) 并将音频文件保存到 `storage/audio` 目录。

## 示例

以下代理将使用 MLX Transcribe 来转录音频文件。
...
agent = Agent(
    ...
    instructions=[
        "要转录音频文件，请使用 `transcribe` 工具，并将音频文件名作为参数。",
        "您可以使用 `read_files` 工具查找所有可用的音频文件。",
    ],
    markdown=True,
)

agent.print_response("总结 Reid Hoffman 的 TED 演讲，并分成几个部分", stream=True)
```
...
| 参数                         | 类型                           | 默认值                           | 描述                                     |
| ------------------------------ | ------------------------------ | -------------------------------- | ---------------------------------------- |
| `base_dir`                     | `Path`                         | `Path.cwd()`                     | 音频文件的基目录                         |
...
| `temperature`                  | `float` or `Tuple[float, ...]` | `None`                           | 采样温度                                 |
| `compression_ratio_threshold`  | `float`                        | `None`                           | 压缩率阈值                               |
| `logprob_threshold`            | `float`                        | `None`                           | 对数概率阈值                             |
| `no_speech_threshold`          | `float`                        | `None`                           | 无语音阈值                               |
| `condition_on_previous_text`   | `bool`                         | `None`                           | 是否基于前文进行条件设置                 |
| `initial_prompt`               | `str`                          | `None`                           | 转录的初始提示                           |
| `word_timestamps`              | `bool`                         | `None`                           | 启用单词级时间戳                         |
| `prepend_punctuations`         | `str`                          | `None`                           | 要前置的标点符号                         |
| `append_punctuations`          | `str`                          | `None`                           | 要后置的标点符号                         |
| `clip_timestamps`              | `str` or `List[float]`         | `None`                           | 剪辑时间戳                               |
