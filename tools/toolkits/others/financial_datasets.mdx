...
**FinancialDatasetsTools** 提供了一个全面的 API，用于检索和分析各种金融数据集，包括来自多个提供商的股票价格、财务报表、公司信息、SEC 文件和加密货币数据。

## 先决条件

该工具包需要一个 Financial Datasets API 密钥，可以通过在 [financialdatasets.ai](https://financialdatasets.ai) 创建账户来获取。
...
更多示例，请参阅 [Financial Datasets 示例](/examples/concepts/tools/others/financial_datasets)。
...
| `enable_company_info`         | `bool`          | `True`  | 启用公司信息相关的函数                                                              |
| `enable_market_data`          | `bool`          | `True`  | 启用市场数据相关的函数（股票价格、收益、指标）                                      |
| `enable_ownership_data`       | `bool`          | `True`  | 启用所有权数据相关的函数（内部交易、机构所有权）                                    |
| `enable_news`                 | `bool`          | `True`  | 启用新闻相关的函数                                                                  |
| `enable_sec_filings`          | `bool`          | `True`  | 启用 SEC 文件相关的函数                                                             |
| `enable_crypto`               | `bool`          | `True`  | 启用加密货币相关的函数                                                              |
| `enable_search`               | `bool`          | `True`  | 启用搜索相关的函数                                                                  |
...
| `get_earnings(ticker: str, limit: int = 10)`                                     | 获取收益报告，包含每股收益估算、实际值和收入数据                                                               |
...
| `get_stock_prices(ticker: str, interval: str = "1d", limit: int = 100)`          | 获取股票历史价格，可配置时间间隔                                                                         |
| `search_tickers(query: str, limit: int = 10)`                                    | 根据查询字符串搜索股票代码                                                                               |
| `get_sec_filings(ticker: str, form_type: Optional[str] = None, limit: int = 50)` | 获取 SEC 文件，可选择按文件类型（如 10-K、10-Q 等）进行筛选                                                 |
| `get_segmented_financials(ticker: str, period: str = "annual", limit: int = 10)` | 获取按产品类别和地理区域划分的细分财务数据                                                               |

## 速率限制和使用

Financial Datasets API 可能根据您的订阅级别有使用限制。有关具体的速率限制信息，请参阅其文档。

## 开发者资源

- 查看 [工具](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/financial_datasets.py)
- 查看 [食谱](https://github.com/agno-agi/agno/blob/main/cookbook/tools/financial_datasets_tools.py)