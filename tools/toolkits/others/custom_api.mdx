...
**CustomApiTools** 使代理能够使用可自定义的身份验证和参数向任何外部 API 发出 HTTP 请求。
...
agent.print_response(
    '向以下两个不同的端点发出 API 调用：/breeds/image/random 和 /breeds/list/all，分别获取随机的狗图片和狗品种列表。两个调用都使用 GET 方法。'
)
```
...
| 参数     | 类型                | 默认值 | 描述                                       |
| -------- | ------------------- | ------ | ------------------------------------------ |
| `base_url`    | `str`               | `None`  | API 调用的基础 URL                                     |
| `username`    | `str`               | `None`  | 基本身份验证的用户名                          |
| `password`    | `str`               | `None`  | 基本身份验证的密码                          |
| `api_key`     | `str`               | `None`  | Bearer 令牌身份验证的 API 密钥                    |
