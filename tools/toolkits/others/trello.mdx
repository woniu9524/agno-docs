---
title: Trello
description: Agno TrelloTools 可帮助将 Trello 功能集成到您的代理中，从而实现对看板、列表和卡片进行管理。
---

## 先决条件

以下示例需要 `trello` 库和 Trello API 凭证，这些凭证可以通过遵循 Trello 的开发者文档获取。
...
以下代理将创建一个名为 `ai-agent` 的看板，并在其中创建名为 `todo` 和 `doing` 的列表，然后在每个列表中创建名为 `create agent` 的卡片。
...
| `get_board_lists` | 检索指定 Trello 看板上的所有列表。                 |
| `move_card`       | 将卡片移动到不同的列表。                                |
| `get_cards`       | 从指定列表中检索所有卡片。                       |
| `create_board`    | 创建新 Trello 看板。                                      |
| `create_list`     | 在指定看板上创建新列表。                         |
| `list_boards`     | 列出已认证用户可访问的所有 Trello 看板。                               |

## 工具包参数

这些参数将传递给 `TrelloTools` 构造函数。
...
`list_boards` 函数接受一个 `board_filter` 参数，该参数具有以下选项：
- `all` (默认)
- `open`
- `closed`
- `organization`
...
- 查看 [Cookbook 示例](https://github.com/agno-agi/agno/blob/main/cookbook/tools/trello_tools.py)