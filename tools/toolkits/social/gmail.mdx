...
**Gmail** 使 Agent 能够与 Gmail 交互，允许其读取、搜索、发送和管理电子邮件。
...
| `search_emails`       | `bool`  | `True`  | 启用使用自然语言查询搜索电子邮件                                   |

## 工具包函数

| 函数               | 描述                                           |
| --------------------- | ---------------------------------------------------------- |
| `get_latest_emails`   | 从用户收件箱获取最新的 X 封电子邮件              |
| `get_emails_from_user`| 从特定发件人获取 X 封电子邮件              |
