...
**TelegramTools** 使 Agent 能够使用 Telegram Bot API 向 Telegram 聊天发送消息。
...
| `chat_id` | `Union[str, int]` | - | 要发送消息的聊天的 ID。 |

## Toolkit 函数

| 函数 | 描述 |
|----------|-------------|
| `send_message` | 向指定的 Telegram 聊天发送消息。接收一个消息字符串作为输入，并以文本形式返回 API 响应。如果发生错误，则返回错误消息。 |
