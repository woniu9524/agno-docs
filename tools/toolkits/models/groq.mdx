...
在使用 `GroqTools` 之前，请确保已安装 `groq` 库并已配置好 Groq API 密钥。
...
2.  **设置 API 密钥：** 从 [Groq Console](https://console.groq.com/keys) 获取您的 API 密钥，并将其设置为环境变量。
...
此示例展示了如何将音频文件（例如法语）翻译成英语，然后从翻译后的文本生成新的音频文件。
...
您可以在初始化时自定义用于转录、翻译和 TTS 的底层 Groq 模型：
...
| 函数           | 描述                                                                 |
| -------------- | -------------------------------------------------------------------- |
| `transcribe_audio` | 使用 Groq Whisper 从本地文件路径或公共 URL 转录音频。 |
| `translate_audio`  | 使用 Groq 将本地文件路径或公共 URL 的音频翻译成英语。 |
| `generate_speech`  | 使用 Groq TTS 从文本生成语音。                               |
