...
**JinaReaderTools** 使 Agent 能够使用 Jina 执行网络搜索任务。
...
下面的 Agent 将使用 Jina API 来总结 https://github.com/AgnoAgi 的内容。
...
| `max_content_length` | `int` | -       | 允许的最大内容长度，从配置中检索。                                       |

## Toolkit 函数

| 函数         | 描述                                                                                                                                                                                           |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `read_url`   | 使用 Jina Reader API 读取指定 URL 的内容。参数包括用于读取 URL 的 `url`。如果请求失败，则返回截断的内容或错误消息。                                                                                   |
| `search_query` | 使用 Jina Reader API 根据指定查询执行网络搜索。参数包括用于搜索词的 `query`。如果请求失败，则返回截断的搜索结果或错误消息。                                                                               |
