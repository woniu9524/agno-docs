---
title: ScrapeGraph
description: Agno ScrapeGraphTools 使代理能够以 markdown 格式从网页中提取结构化数据供 LLM 使用。
---
...
如果您的 ScrapeGraph 配置或特定模型需要 API 密钥，请选择性地设置 `SGAI_API_KEY` 环境变量：
...
以下代理使用 `ScrapeGraphTools` 通过 `smartscraper` 功能从网页中提取特定信息。
...
agent.print_response("""
    "使用 smartscraper 从 https://www.wired.com/category/science/ 提取以下内容：
- 新闻文章
- 标题
- 图片
- 链接
...
| 参数      | 类型            | 默认值 | 描述                                                                                                                             |
| -------------- | --------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `api_key`      | `Optional[str]` | `None`  | ScrapeGraph 服务的 API 密钥。如果未提供，则默认为 `SGAI_API_KEY` 环境变量。                             |
