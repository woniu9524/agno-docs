...
**SQLTools** 使 Agent 能够运行 SQL 查询并与数据库进行交互。
...
您还需要安装用于您打算使用的特定数据库的相应 Python 适配器。
...
```
`mysqlclient` 适配器可能需要额外的系统级依赖项。请参阅[官方安装指南](https://github.com/PyMySQL/mysqlclient/blob/main/README.md#install)了解更多详细信息。
...
以下 Agent 将运行一个 SQL 查询来列出数据库中的所有表，并描述其中一个表的内容。
...
| `user`           | `str`            | -       | 用于数据库身份验证的用户名。                                   |
| `password`       | `str`            | -       | 用于数据库身份验证的密码。                                   |
| `host`           | `str`            | -       | 数据库服务器的主机名或 IP 地址。                          |
| `port`           | `int`            | -       | 数据库服务器正在监听的端口号。                  |
| `schema`         | `str`            | -       | 要使用的数据库中的特定模式。                             |
