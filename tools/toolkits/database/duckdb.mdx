...
agent = Agent(
    ...
    system_message="使用此文件获取电影数据：https://agno-public.s3.amazonaws.com/demo_data/IMDB-Movie-Data.csv",
)
...
| `run_queries` | `bool`               | `True` | 确定在操作期间是否运行 SQL 查询。                                    |
...
| `inspect_query`        | 检查查询并返回查询计划的函数。在运行查询之前，请务必检查您的查询。                                                                                                                                                                                           |
| `run_query`            | 运行查询并返回结果的函数。                                                                                                                                                                                                                                   |
| `summarize_table`      | 计算表中聚合值的函数。该函数启动一个查询，该查询计算所有列的聚合值，包括 min、max、avg、std 和 approx_unique。                                                                                                                                                 |
| `get_table_name_from_path` | 从路径获取表名的函数                                                                                                                                                                                                                                         |
| `create_table_from_path` | 从路径创建表的函数                                                                                                                                                                                                                                           |
| `export_table_to_path` | 以所需格式（默认为 parquet）保存表的函数。如果提供了路径，表将保存在该路径下。例如：如果路径是 /tmp，表将保存为 /tmp/table.parquet。否则，它将保存在当前目录中。 |
| `load_local_path_to_table` | 将本地文件加载到 duckdb 的函数                                                                                                                                                                                                                               |
| `load_local_csv_to_table`  | 将本地 CSV 文件加载到 duckdb 的函数                                                                                                                                                                                                                          |
| `load_s3_path_to_table`    | 从 S3 加载文件的函数到 duckdb                                                                                                                                                                                                                                |
| `load_s3_csv_to_table`     | 从 S3 加载 CSV 文件的函数到 duckdb                                                                                                                                                                                                                           |
| `create_fts_index`         | 在表上创建全文搜索索引的函数                                                                                                                                                                                                                                 |
| `full_text_search`         | 在表列中搜索特定文本/关键字的全文搜索函数                                                                                                                                                                                                                  |
