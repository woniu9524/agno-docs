...
Agno Agents 可以并发执行多个工具，让您能够高效地处理模型生成的函数调用。当函数涉及耗时操作时，这一点尤为重要。这可以提高响应速度并缩短总体执行时间。
...
1. 为您的 Agent 提供一个工具列表，最好是异步工具以获得最佳性能。但是，也可以使用同步函数，因为它们将在单独的线程上并发执行。
2. 使用 `arun` 或 `aprint_response` 方法运行 Agent，以启用工具调用的并发执行。

<Note>
  工具的并发执行需要支持并行函数调用的模型。例如，OpenAI 模型有一个
  `parallel_tool_calls` 参数（默认启用），允许同时请求和执行多个工具调用。
</Note>

在此示例中，`gpt-4o` 对 `atask1`、`atask2` 和 `atask3` 发出了三个同时的工具调用。通常，这些工具调用会顺序执行，但使用 `aprint_response` 函数，它们会并发运行，从而提高了执行时间。
