...
如果在工具调用后需要“重试”模型，并提供不同的指令或停止代理，我们可以引发以下异常之一：

- `RetryAgentRun`：当您希望使用不同的指令重试代理运行时，请使用此异常。
- `StopAgentRun`：当您希望停止代理运行时，请使用此异常。
- `AgentRunException`：一个通用异常，可用于重试工具调用。


此示例展示了如何使用 `RetryAgentRun` 异常来使用其他指令重试代理。
...
def add_item(agent: Agent, item: str) -> str:
    ...
    if len_shopping_list < 3:
        raise RetryAgentRun(
            f"购物清单是：{agent.session_state['shopping_list']}。购物清单最少需要 3 件商品。 "
            + f"请再添加 {3 - len_shopping_list} 件商品。",
        )
...
<Tip>
请确保将 `AGNO_DEBUG` 设置为 `True` 以查看调试日志。
</Tip>