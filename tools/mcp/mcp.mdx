...
title: 模型上下文协议 (MCP)
sidebarTitle: 概述
description: 了解如何使用 Agno 的 MCP 来使您的代理通过标准化接口与外部系统进行交互。
---
...
<Steps>
    <Step title="查找您想使用的 MCP 服务器">
    您可以使用任何可用的 MCP 服务器。要查看一些示例，您可以查看 MCP 维护者自己的 [此 GitHub 存储库](https://github.com/modelcontextprotocol/servers)。
    </Step>

    <Step title="初始化 MCP 集成">
    将 `MCPTools` 类初始化为上下文管理器。定义 MCP 服务器的推荐方法是使用 `command` 或 `url` 参数。使用 `command`，您可以传递用于运行您想要的 MCP 服务器的命令。使用 `url`，您可以传递您想要使用的正在运行的 MCP 服务器的 URL。
    ...
    <Step title="将 MCPTools 提供给 Agent">
    在初始化 Agent 时，将 `MCPTools` 类传递给 `tools` 参数。
...
这是一个文件系统 Agent，它使用 [Filesystem MCP 服务器](https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem) 来浏览和分析文件：
...
您还可以在 Agno Playground 中运行 MCP 服务器，它提供了一个与您的代理交互的 Web 界面。以下是在 Playground 中运行的 GitHub Agent 示例：
...
1. **错误处理**：始终包含 MCP 服务器连接和操作的适当错误处理。

2. **资源清理**：使用 `MCPTools` 或 `MultiMCPTools` 作为异步上下文管理器，以确保正确清理资源：
```python
async with MCPTools(command) as mcp_tools:
    # 您的 Agent 代码在此处
```

3. **清晰的说明**：为您的 Agent 提供清晰具体的说明：
...
- 在此处查找使用 MCP 的 Agent 示例：[https://docs.agno.com/examples/concepts/tools/mcp/airbnb](https://docs.agno.com/examples/concepts/tools/mcp/airbnb)。
...
- 查看 Agno [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/tools/mcp) 以获取更多使用 MCP 的 Agent 示例。