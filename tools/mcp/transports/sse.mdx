...
Agno 的 MCP 集成支持 [SSE 传输](https://modelcontextprotocol.io/docs/concepts/transports#server-sent-events-sse)。此传输支持服务器到客户端的流式传输，在受限网络中工作时可能比 [stdio](https://modelcontextprotocol.io/docs/concepts/transports#standard-input%2Foutput-stdio) 更有用。
...
您还可以使用 `server_params` 参数来定义 MCP 连接。这样，您可以为每次请求指定要发送到 MCP 服务器的标头以及超时值：
...
<Steps>
    ...
    <Step title="设置客户端">
        ...
        # 在此示例中，我们连接到我们的示例服务器（SSE 传输）和另一个服务器（stdio 传输）。
