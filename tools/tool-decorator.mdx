...
在大多数生产场景中，您都需要编写自己的工具。这就是为什么我们专注于在 Agno 中提供最佳的工具使用体验。

规则很简单：

- 任何 Python 函数都可以被 Agent 用作工具。
- 使用 `@tool` 装饰器来修改此工具调用之前和之后发生的事情。
...
agent.print_response("总结一下 Hacker News 的前 5 条新闻？", stream=True)
```
...
- `external_execution=True`: 工具将在代理的控制之外执行。
...
def logger_hook(function_name: str, function_call: Callable, arguments: Dict[str, Any]):
    ...
    print(f"函数调用已完成，结果为：{result}")
    return result

@tool(
    name="fetch_hackernews_stories",                # 工具的自定义名称（否则使用函数名）
    description="获取 Hacker News 的热门新闻",  # 自定义描述（否则使用函数文档字符串）
    show_result=True,                               # 在函数调用后显示结果
    stop_after_tool_call=True,                      # 在工具调用后立即返回结果并停止 Agent
    tool_hooks=[logger_hook],                       # 在执行之前和之后运行的钩子
    requires_confirmation=True,                     # 在执行前需要用户确认
    cache_results=True,                             # 启用结果缓存
    cache_dir="/tmp/agno_cache",                    # 自定义缓存目录
    cache_ttl=3600                                  # 缓存的生存时间（秒）（1 小时）
)
...
| `requires_confirmation` | `bool` | 如果为 True，在执行前需要用户确认 |
| `requires_user_input` | `bool` | 如果为 True，在执行前需要用户输入 |
| `user_input_fields` | `list[str]` | 需要用户输入的字段列表 |
| `external_execution` | `bool` | 如果为 True，工具将在代理的控制之外执行 |
| `cache_results` | `bool` | 如果为 True，启用函数结果的缓存 |
| `cache_dir` | `str` | 用于存储缓存文件的目录 |
| `cache_ttl` | `int` | 缓存结果的生存时间（秒）（默认为 3600） |