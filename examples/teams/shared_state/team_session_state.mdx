...
此示例演示了共享的 `team_session_state` 如何在嵌套代理和子团队之间传播和持久化，从而实现协作任务的无缝状态管理。
...
shopping_list_agent = Agent(
    ...
    instructions=[
        "通过添加和移除商品来管理购物列表",
        "添加或移除商品时请始终确认",
        "如果任务已完成，请更新会话状态以记录所做的更改和已完成的杂务",
    ],
)
...
shopping_mgmt_team = Team(
    ...
    instructions=[
        "使用购物列表代理管理购物列表的添加和移除操作",
        "将添加或移除商品的请求转发给购物列表代理",
    ],
)
...
recipe_agent = Agent(
    ...
    instructions=[
        "首先，使用 get_ingredients 工具获取购物列表中的当前食材",
        "获取食材后，根据这些食材创建详细的菜谱建议",
        "使用可用食材创建至少 3 种不同的菜谱创意",
        "每种菜谱应包括：名称、所需食材（突出显示来自购物列表的食材）和简要的准备步骤",
        "在菜谱建议方面要富有创意但要切合实际",
        "考虑人们通常除了购物列表中的食材外，还会有的常见食品柜物品",
        "如果用户提到了饮食偏好，请予以考虑",
        "如果没有指定餐点类型，请建议多种选项（早餐、午餐、晚餐、零食）",
    ],
)
...
meal_planning_team = Team(
    ...
    instructions=[
        "您是一个根据购物列表中的食材建议菜谱的膳食计划团队。",
        "重要提示：当用户询问“我可以用这些食材做什么？”或任何与菜谱相关的问题时，请立即将完全相同的请求转发给菜谱代理，无需进一步询问。",
        "不要向用户索要食材 - 菜谱代理将使用购物列表中已有的食材。",
        "您的主要工作是直接将菜谱请求转发给菜谱代理，不做任何修改。",
    ],
)
...
def add_chore(team: Team, chore: str, priority: str = "medium") -> str:
    ...
    Args:
        chore (str): 要添加到列表中的杂务。
        priority (str): 杂务的优先级级别（低、中、高）。
    ...
    return f"已添加杂务：'{chore}'，优先级为 {priority}"
...
shopping_team = Team(
    ...
    markdown=True,
    instructions=[
        "您是一个管理购物列表并帮助根据该列表规划膳食的团队。",
        "如果您需要向购物列表中添加或移除商品，请将完整的请求转发给购物管理团队。",
        "重要提示：如果用户询问有关菜谱或他们可以用食材做什么的问题，请立即将完全相同的请求转发给膳食计划团队，不做任何额外询问。",
        "例如：当用户询问“我可以用这些食材做什么？”时，您应该直接将此确切请求转发给膳食计划团队，而无需询问更多信息。",
        "如果您需要列出购物列表中的商品，请使用 list_items 工具。",
        "如果用户从购物列表中获取了某件商品，这意味着可以将其从购物列表中移除。",
        "每次完成任务后，请使用 add_chore 工具以高优先级记录所完成的操作。",
    ],
...
# 尝试膳食计划功能
shopping_team.print_response("我可以用这些食材做什么？", stream=True)
print(f"会话状态：{shopping_team.team_session_state}")

shopping_team.print_response(
    "清除列表中的所有内容，然后重新开始，只保留香蕉和酸奶",
    stream=True,
)
