...
このワークフローは、営業担当者が以下のステップを通じて、高度にパーソナライズされたコールドメールを作成するのに役立ちます。
1. ウェブサイトを通じてターゲット企業をリサーチする
2. ビジネスモデル、技術スタック、固有の属性を分析する
3. パーソナライズされたメールの下書きを生成する
4. 実際の連絡前にレビューするために、テストメールを自分自身に送信する

なぜこれが役立つのか？
--------------------------------------------------------------------------------
• 常に余分なレビュー手順があります—メールは最初にあなたに送信されます。
  これにより、実際の潜在顧客に連絡する前にメッセージングを微調整できます。
• トーン、スタイル、パーソナライゼーションを大量に反復処理するのに理想的です。

誰が使うべきか？
--------------------------------------------------------------------------------
• SDR、アカウントエグゼクティブ、事業開発マネージャー
• 創業者、マーケティング担当者、B2B営業担当者
• 関係構築や大規模なアウトリーチを行っているすべての人
...
3. sender_details_dict を YOUR 情報で更新します。

4. ターゲット企業を "leads" ディクショナリに追加します。

5. 実行します：
   python personalized_email_generator.py

スクリプトは、DEMO_MODE=False の場合、最初にメールをコンソールに送信します。
DEMO_MODE=True の場合、レビューのためにコンソールにメールを出力します。

その後、洗練されたメールを自信を持って潜在顧客に送信できます！
