...
description: "一个异步 Hacker News 报告器，使用工作流获取最新新闻"
---

**AsyncHackerNewsReporter** 是一个旨在异步获取 Hacker News 热门新闻并生成综合报告的工作流。此工作流利用 `arun` 方法高效处理多个异步任务，确保操作流畅且不阻塞。
...
class AsyncHackerNewsReporter(Workflow):
    ...
    writer: Agent = Agent(
        ...
        instructions=[
            ...
            "确保标题吸引人且引人入胜。",
            "分享每篇文章的分数、标题、网址和摘要。",
            "为各部分提供相关的标题，并在每个部分提供详细信息/事实/流程。",
            "忽略无法阅读或理解的文章。",
