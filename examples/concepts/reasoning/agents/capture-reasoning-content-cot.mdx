...
此示例演示了在使用 `reasoning=True` 或设置特定的 `reasoning_model` 时，如何访问和打印 `reasoning_content`。
## 代码
...
# 创建 reasoning=True 的代理（默认模型 COT）
agent = Agent(
...
print("\n--- response 中的 reasoning_content ---")
if hasattr(response, "reasoning_content") and response.reasoning_content:
    print(response.reasoning_content)
else:
    print("在 response 中未找到 reasoning_content")
...
# 在流式处理后从代理的 run_response 访问 reasoning_content
print("\n--- 流式处理后 agent.run_response 中的 reasoning_content ---")
if (
    ...
    print("在流式处理后 agent.run_response 中未找到 reasoning_content")


print("\n\n=== 示例 4：流式处理 reasoning_model ===\n")

# 为流式处理创建一个新的、带有 reasoning_model 的代理
