...
Agno は、`SingleStoreStorage` クラスを使用して、Teams のストレージバックエンドとして Singlestore をサポートしています。
...
# SSL_CERT が提供されていない場合は証明書をダウンロード
if not SSL_CERT:
...
- [クックブック](https://github.com/agno-agi/agno/blob/main/cookbook/storage/singlestore_storage/singlestore_storage_for_team.py)を表示