...
Agno は、`PostgresStorage` クラスを使用して、PostgreSQL をワークフローのストレージバックエンドとしてサポートしています。

## 使用方法

### PgVector の実行

[docker desktop](https://docs.docker.com/desktop/install/mac-install/) をインストールし、以下のコマンドを使用してポート **5532** で **PgVector** を実行します。
...
- [Cookbook](https://github.com/agno-agi/agno/blob/main/cookbook/storage/postgres_storage/postgres_storage_for_workflow.py) を表示