---
title: 会议摘要代理
description: 多模态Agno代理，可转录会议录音、提取关键见解、生成视觉摘要和使用OpenAI工具创建音频摘要。
---

本示例演示了一个多模态会议摘要和可视化代理，它使用OpenAITools和ReasoningTools来转录会议录音、提取关键见解、生成视觉摘要和合成音频摘要。
...
该脚本使用OpenAITools（transcribe_audio, generate_image, generate_speech）
来处理会议录音、进行摘要、可视化并创建音频摘要。
...
print(f"正在将文件下载到本地路径：{local_audio_path}")
...
meeting_agent: Agent = Agent(
    ...
    instructions=dedent(f"""\
        ...
        3. 分析转录内容，撰写一份简洁的摘要，突出关键的讨论点、决策和行动项。
        4. 基于步骤3中创建的摘要，生成格式化的会议纪要。
