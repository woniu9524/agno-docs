...
使用 [Notion MCP 服务器](https://github.com/makenotion/notion-mcp-server) 创建一个可以创建、更新和搜索 Notion 页面的 Agent：
...
3. 连接相关的 Notion 页面到该集成。为此，您需要访问该页面，点击三个点，然后选择“连接到集成”。
...
async def run_agent():
    ...
    async with MCPTools(server_params=server_params) as mcp_tools:
        ...
        await agent.acli_app(
            message="您是一个可以访问 Notion 工作区和页面的有用助手。",
            stream=True,
            markdown=True,
            exit_on=["exit", "quit"],
        )
