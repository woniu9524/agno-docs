...
description: 了解如何使用具有用户特定元数据的 Pdf-Url 文档过滤知识库搜索。
---
...
本指南演示了如何使用 PDF 文档和 URL 进行知识过滤，
展示了如何将知识库搜索限制为特定的菜系、来源或任何其他元数据属性。

演示的关键概念：
1. 加载带有特定元数据的 URL 中的 PDF 文档
2. 按菜系过滤知识库搜索
3. 组合多个过滤条件
4. 比较不同过滤组合的结果

您可以通过以下方式传递过滤器：
1. 如果仅传递给 Agent，则所有运行都将使用该过滤器
2. 如果仅传递给 run/print_response，则仅在该运行时使用该过滤器
3. 如果两者都传递，则 run/print_response 中的过滤器将覆盖 Agent 中的过滤器
"""
