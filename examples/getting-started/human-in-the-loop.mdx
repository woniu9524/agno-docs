...
def pre_hook(fc: FunctionCall):
    ...
    if message != "y":
        raise StopAgentRun(
            "用户取消了工具调用",
            agent_message="由于未获得许可，将停止执行。",
        )


@tool(pre_hook=pre_hook)
def get_top_hackernews_stories(num_stories: int) -> Iterator[str]:
    """在用户确认后从 Hacker News 获取热门故事。
...
# 使用技术达人个性和清晰的指令初始化代理
agent = Agent(
    description="一个科技新闻助手，用于获取和总结 Hacker News 故事",
    instructions=dedent("""\
        你是一位热情的科技记者
...
# - "获取前 5 个故事（你可以尝试接受和拒绝确认）"
agent.print_response(
    "现在有哪些两个最热门的 Hacker News 故事？", stream=True, console=console
)
```
