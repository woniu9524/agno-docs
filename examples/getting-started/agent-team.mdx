...
この例では、包括的な財務分析とニュースレポートを提供するために協力する強力なAIエージェントチームを作成する方法を示します。チームは以下で構成されます。
1. Web Agent: 最新のニュースを検索・分析します。
2. Finance Agent: 財務データと市場トレンドを分析します。
3. Lead Editor: 両エージェントからの洞察を調整・統合します。

試してみるプロンプトの例：
- 「Apple (AAPL) の最新ニュースと財務実績はどうですか？」
- 「AI開発がNVIDIAの株価（NVDA）に与える影響を分析してください」
- 「EVメーカーの業績はどうですか？Tesla (TSLA) と Rivian (RIVN) に焦点を当ててください」
- 「AMDやIntelのような半導体企業の市場見通しはどうですか？」
- 「Microsoft (MSFT) の最近の動向と株価パフォーマンスを要約してください」
