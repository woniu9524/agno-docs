...
<Steps>
  ...
  <Step title="设置安装了 pgvector 的 PostgreSQL">
    您需要一个安装了 pgvector 扩展的 PostgreSQL 数据库。请根据您的数据库配置调整代码中的 `db_url`。
  </Step>
  ...
  <Step title="后续运行">
    首次运行后，请注释掉 `knowledge_base.load(recreate=True)` 行，以避免重新加载 PDF。
  </Step>
</Steps>

此示例展示了如何将知识库与 IBM WatsonX 集成。它从 URL 加载 PDF，将其处理到向量数据库（本例中为带有 pgvector 的 PostgreSQL），然后创建一个可以查询此知识库的 Agent。
