...
所有工作流都带有一个 `session_state` 字典，您可以使用它来缓存中间结果。`session_state` 绑定到 `session_id`，并且可以持久化到数据库。

为您的工作流提供 `storage` 以支持在数据库中持久化会话状态。
...
然后在 `run()` 方法中，您可以根据需要从 `session_state` 读取和添加内容。
...
当工作流启动时，该特定 `session_id` 的 `session_state` 会从数据库中读取，当工作流结束时，`session_state` 会存储到数据库中。
