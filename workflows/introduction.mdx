...
In our experience building AI products, workflow logic generally needs to be dynamic (i.e. determined at runtime) and requires fine-grained control over parallelization, caching, state management, error handling, and issue resolution.

A custom abstraction would be a new DSL meaning new concepts to learn and more code to write.

Which project we've worked on, a simple python function always seems to do the trick. We also found that complex workflows can span multiple files, sometimes turning into modules in themselves. You know what works great here? Python.
...
For these reasons, we recommend building workflows as simple python functions, the level of control, flexibility and reliability is unmatched.
