...
3. Implement the workflow logic in the `run()` method. This is the main function that will be called when you run the workflow aka **the workflow entrypoint**. This function gives us so much control over the process, some agents can stream, other's can generate structured outputs, agents can be run in parallel using `async.gather()`, some agents can have validation logic that runs before returning the response.
...
**Why did we implement workflow logic as a simple python function instead of creating a custom abstraction like Graph, Chain, or Flow?**

In our experience building AI products, workflow logic typically needs to be dynamic (i.e. determined at runtime) and requires fine-grained control over parallelization, caching, state management, error handling, and issue resolution.

It was simpler just to use python.

Another significant challenge with long-running workflows is managing request/response timeouts. We needed workflows to trigger asynchronously, respond to the client confirming initiation, and then allow the client to poll for results later. Achieving this UX required precise control over the workflow’s yielding of "RunResponse" objects and granular state management.

Framework-based approaches inherently abstract away low-level control, that is literally the point of the framework. Since these frameworks are themselves built upon Python, they can never offer more control than Python itself. Furthermore, the moment you need something custom, you need to learn the framework's "language", "concepts" and how to work around them.

We didn't want to create our own language, we just wanted to use python.
