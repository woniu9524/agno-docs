...
[Docker](https://docs.docker.com/get-started/overview/) 是一项革命性的技术，它使我们能够本地运行应用程序。我们将 [Apps](/workspaces/apps/introduction) 打包到包含运行应用程序所需一切的**容器**中。

## Docker 资源

Agno 使我们能够将 Docker 资源定义为 Pydantic 对象，以便我们能够纯粹地用 Python 构建应用程序层。在大多数情况下，您不会直接创建 **Docker 资源**，而是使用 [Apps](/workspaces/apps/introduction) 为我们创建资源。

<Accordion title="与 docker-compose 的区别">

Docker Compose 更加成熟，它将资源定义为“YAML 配置”，而 Agno 允许我们将资源定义为“Python 对象”。

我们仍然喜欢 docker-compose，只是想提升体验。使用 Agno，我们很少直接定义资源，而是使用 Apps 为我们创建资源。
...
- 允许重用和测试资源。
- 将它们作为常规 Python 对象导入到软件层中。
- 将多个资源打包到 [Apps](/workspaces/apps/introduction) 中，以便我们可以定义**“应用程序即代码”**。
- 实现与 Python 代码中的资源交互的 AI 功能。
