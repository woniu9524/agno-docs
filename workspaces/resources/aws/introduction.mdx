...
AWS 资源使我们能够将 AWS 服务创建为 pydantic 对象，从而完成了我们完全用 Python 编写软件、应用程序和基础设施代码的愿景。

## 示例

### S3 存储桶

将以下代码复制到 `resources.py` 文件中，然后运行 `ag start resources.py` 来创建一个名为 `my-bucket-885` 的存储桶。
...
请确保使用 `ag stop resources.py` 删除该存储桶。

### Secret Manager

将以下代码复制到 `resources.py` 文件中，然后运行 `ag start resources.py` 来创建一个名为 `my-secret` 的密钥。
