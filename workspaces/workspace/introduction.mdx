...
A agno template creates a **Workspace**, which is just an umbrella term for your codebase.

## Create new workspace

Run `ag ws create` to create a new workspace using an Agno template
...
```bash Create AI App
ag ws create -t ai-app -n ai-app
```

```bash Create AI Api
ag ws create -t api-app -n api-app
```

```bash Create Django App
ag ws create -t django-app -n web-app
```
...
`ag` will ask for a workspace template and name if not provided.

</Note>

## Setup existing workspace

Run `ag ws setup` to setup an existing directory as an Agno workspace
...
Run `ag ws up` to create workspace resources
...
Run `ag ws down` to delete workspace resources
...
Run `ag ws patch` to update workspace resources
