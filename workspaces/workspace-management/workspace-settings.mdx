...
`workspace/settings.py` 文件中的 `WorkspaceSettings` 对象定义了您的应用程序和资源使用的通用设置。以下是我们建议更新的设置：
...
`ws_name` 用于命名您的应用程序和资源。将其更改为您的项目或团队名称，例如：
...
- 您的应用程序镜像
- 数据库、Streamlit 应用程序和 FastAPI 服务器等应用程序
- 存储桶、密钥和负载均衡器等资源
...
将 `build_images` 设置为 `True` 将在运行 `ag ws up dev:docker` 或 `ag ws up prd:docker` 时在本地构建镜像。
...
将 `push_images` 设置为 `True` 将在运行 `ag ws up dev:docker` 或 `ag ws up prd:docker` 时在构建后推送镜像。
