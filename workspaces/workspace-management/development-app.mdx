...
你的开发应用在 Docker 中本地运行，其资源定义在 `workspace/dev_resources.py` 文件中。本指南将展示如何：
...
`workspace/settings.py` 文件中的 `WorkspaceSettings` 对象定义了你的工作区应用和资源常用的设置。

## 构建你的开发镜像

默认情况下，你的应用使用 `agno` 镜像。要使用你自己的镜像：
