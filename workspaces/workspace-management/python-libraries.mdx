...
Agno 模板使用 `pyproject.toml` 文件来管理依赖项，**该文件用于使用 [pip-tools](https://pip-tools.readthedocs.io/en/latest/) 生成 `requirements.txt` 文件。**

添加或更新 Python 库是一个包含 2 个步骤的过程：

1. 将库添加到 `pyproject.toml` 文件中
2. 自动生成 `requirements.txt` 文件

<Warning>

强烈建议通过此过程自动生成 `requirements.txt` 文件。
