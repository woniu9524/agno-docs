...
当我们谈论记忆时，通常的理解是存储关于 Agent 正在交互的用户的信息和事实的能力。简而言之，就是建立用户的画像，了解他们的偏好，并以此来个性化 Agent 的响应。

## Agentic Memory

Agno Agents 原生支持 Agentic Memory Management，并推荐将其作为您记忆之旅的起点。

通过 Agentic Memory，Agent 本身会创建、更新和删除用户对话中的记忆。

设置 `enable_agentic_memory=True`，为 Agent 提供一个管理用户记忆的工具，该工具会将任务传递给 `MemoryManager` 类。

> 您也可以设置 `enable_user_memories=True`，它会在每次用户消息后始终运行 `MemoryManager`。[参见下面的示例。](#create-memories-after-each-run)
...
- `read_chat_history=True` 会向 Agent 添加一个允许其读取聊天记录的工具，因为聊天记录可能比 `num_history_runs` 中包含的内容更多。

## 每次运行后创建记忆

虽然 `enable_agentic_memory=True` 为 Agent 提供了一个管理用户记忆的工具，但我们也可以在每次用户消息后“触发” `MemoryManagement`。

设置 `enable_user_memories=True`，它会在每次用户消息后始终处理记忆。
...
agent = Agent(
    ...
    memory=memory,
    # 这将在每次用户消息后触发 MemoryManager
    enable_user_memories=True,
)

# 发送一条需要使用记忆的消息给 Agent
...
Agno 中的 `Memory` 类允许您管理用户记忆的各个方面。让我们从在 Agent 之外使用 `Memory` 的一些示例开始。我们将：
...
您可以使用时间顺序方法检索记忆，例如 `last_n`（最近的）或 `first_n`（最旧的优先）：
...
Agentic 搜索允许您根据含义查找记忆，而不是精确的关键字匹配。这对于检索上下文相关的非常有用：

```python
from agno.memory.v2.memory import Memory, UserMemory
from agno.models.google.gemini import Gemini

# 使用模型初始化记忆以进行 agentic 搜索
...
通过 agentic 搜索，模型可以理解您查询背后的意图，并返回最相关的记忆，即使它们不包含您搜索中的确切关键字。
