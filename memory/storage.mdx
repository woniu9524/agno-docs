...
如果您在生产环境中使用内存，持久化存储至关重要，因为您希望在应用程序重启后保留用户记忆。

Agno 的内存系统支持多种持久化存储选项。

## 存储选项

Memory 类通过可插拔的数据库接口支持不同的后端存储选项。目前，Agno 提供：
...
要配置内存存储，您需要创建一个数据库实例并将其传递给 Memory 构造函数：
...
使用持久化存储时，Memory 系统会存储：

- **用户记忆** - 关于用户的见解和事实
- **最后更新时间戳** - 用于跟踪记忆最后修改的时间
- **记忆 ID** - 每个记忆的唯一标识符
...
在构建带有内存的代理时，您通常希望同时存储代理会话和记忆。Agno 通过允许您配置这两个存储系统，使此过程变得简单：
...
使用持久化存储时，Memory 系统提供了几个函数来管理存储的记忆：
...
- 在此处查找内存存储的参考文档：[/reference/memory/storage](/reference/memory/storage)